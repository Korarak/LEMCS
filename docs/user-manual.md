...
> **เวอร์ชัน**: 1.2 | **ปรับปรุงล่าสุด**: เมษายน 2569  
...
11. [รายงาน](#11-รายงาน)
12. [กรอกแบบประเมินชั้นเรียน (สำหรับครู)](#12-กรอกแบบประเมินชั้นเรียน-สำหรับครู)
13. [Audit Log](#13-audit-log)
14. [สิทธิ์การเข้าถึงตามบทบาท (RBAC)](#14-สิทธิ์การเข้าถึงตามบทบาท-rbac)
15. [คำถามที่พบบ่อย](#15-คำถามที่พบบ่อย)
...
|-----------|--------------|---------|
| ST-5 | นักเรียนอายุ **15 ปีขึ้นไป** | ระดับความเครียด |
| CDI | นักเรียนอายุ **7–17 ปี** | ภาวะซึมเศร้าในเด็ก |
| PHQ-A | นักเรียนอายุ **11–20 ปี** | ภาวะซึมเศร้าในวัยรุ่น |

> **หมายเหตุ:** ช่วงอายุที่ทับซ้อนกัน — อายุ 15–17 ปี ทำได้ทั้ง 3 แบบ | อายุ 11–14 ปี ทำ CDI + PHQ-A | อายุ < 11 ปี ทำ CDI เท่านั้น | อายุ < 7 ปี หรือไม่มีข้อมูลวันเกิด ไม่แสดงแบบประเมิน
...
📸 SCREENSHOT-23: ปุ่มแก้ไข/ลบในตาราง
    ...
    — ปุ่ม ✏️ แก้ไขข้อมูลทั่วไป
    — ปุ่ม 🪪 แก้ไขเลขบัตรประชาชน (เฉพาะ systemadmin)
    — ปุ่ม 🚫/✅ ปิด/เปิดบัญชี — มี Confirm Dialog ก่อนดำเนินการ
```

### 7.6 แก้ไขเลขบัตรประชาชน (PDPA)

กรณีนำเข้าข้อมูลผิดพลาด สามารถแก้ไขเลขบัตรประชาชนของนักเรียนได้ด้วยปุ่ม 🪪

```
📸 SCREENSHOT-23b: Modal แก้ไขเลขบัตรประชาชน
    [แทรกภาพ: screenshots/23b-national-id-edit.png]
    — แบนเนอร์เตือน PDPA สีเหลือง
    — ช่องกรอกเลขบัตรใหม่ (ตรวจสอบ checksum อัตโนมัติ)
    — ขั้นตอน 2 ขั้น: ตรวจสอบ → ยืนยัน
    — รองรับทั้งเลขบัตรไทย 13 หลัก และ G-Code (นักเรียนไร้สัญชาติ)
```

> ⚠️ การแก้ไขเลขบัตรประชาชนจะถูกบันทึกใน Audit Log ทุกครั้งตาม พ.ร.บ. PDPA
...
## 12. กรอกแบบประเมินชั้นเรียน (สำหรับครู)

เมนูนี้ออกแบบสำหรับครูโรงเรียนประถมศึกษา ที่ต้องสัมภาษณ์นักเรียนแล้วกรอกคำตอบแทน (เนื่องจากนักเรียนอายุน้อยยังไม่สามารถใช้ระบบเองได้) — เข้าถึงได้เฉพาะ **schooladmin** เท่านั้น

### 12.1 เข้าใช้เมนู

กดเมนู **"📋 กรอกแบบประเมินชั้นเรียน"** ในแถบด้านซ้าย

### 12.2 เลือกชั้นเรียน

```
📸 SCREENSHOT-37: เลือกชั้นและห้อง
    [แทรกภาพ: screenshots/37-proxy-class-select.png]
    — Dropdown เลือกชั้น (ป.1–ม.6)
    — ช่องกรอกห้อง
    — Progress bar แสดง X/Y คน ทำแล้ว
```

### 12.3 รายชื่อนักเรียน

```
📸 SCREENSHOT-38: ตารางรายชื่อพร้อมสถานะ
    [แทรกภาพ: screenshots/38-proxy-student-list.png]
    — แสดงสถานะแต่ละแบบประเมิน (CDI / PHQ-A / ST-5) ต่อนักเรียน
    — ✅ ทำแล้ว (badge สี), ⬜ ยังไม่ทำ
    — ปุ่มลัดต่อแบบประเมินแต่ละประเภท
    — ✏️ icon หมายความว่า ครูเป็นผู้กรอกให้
```

### 12.4 กรอกแบบประเมิน

1. กดปุ่มชื่อแบบประเมิน เช่น **[CDI]** ในแถวนักเรียน
2. ยืนยัน PDPA — ระบบแสดงชื่อนักเรียนและแจ้งว่าจะบันทึกชื่อครูใน audit log
3. ถามนักเรียนทีละข้อ กดเลือกคำตอบตามที่นักเรียนตอบ
4. ใช้ปุ่ม dot navigator (●●○○...) กดข้ามข้อที่ต้องการทบทวน
5. กด **"ส่งคำตอบ"** เมื่อตอบครบทุกข้อ
6. ดูผลทันที — หากพบความเสี่ยงวิกฤต ระบบแจ้งเตือนทันที
7. กด **"← กลับรายชื่อ"** เพื่อทำนักเรียนคนถัดไป

> ⚠️ **PDPA**: การกรอกแทนจะถูกบันทึกว่า "กรอกโดย [ชื่อครู]" ใน audit log ทุกครั้ง  
> ✅ **กรอกซ้ำได้**: หากต้องการแก้ไข สามารถกรอกใหม่ได้ ระบบจะบันทึกทุก session

---

## 13. Audit Log

เมนู **"🔍 Audit Log"** แสดงบันทึกการกระทำที่ส่งผลต่อข้อมูลสำคัญ — เข้าถึงได้เฉพาะ **systemadmin** และ **superadmin**

### 13.1 ข้อมูลที่บันทึก

| Action | เมื่อไหร่ |
|--------|---------|
| `view_alert` | เปิดดูรายละเอียด alert |
| `update_alert` | อัปเดตสถานะ/หมายเหตุ alert |
| `update_national_id` | แก้ไขเลขบัตรประชาชนนักเรียน |
| `proxy_assessment` | ครูกรอกแบบประเมินแทนนักเรียน |

### 13.2 การกรองข้อมูล

สามารถกรองตาม: Action ประเภท, Resource (alert/student), ช่วงวันที่

---

## 14. สิทธิ์การเข้าถึงตามบทบาท (RBAC)

| เมนู / ฟีเจอร์ | systemadmin | superadmin | commissionadmin | schooladmin |
|--------------|:-----------:|:----------:|:---------------:|:-----------:|
| Dashboard | ✅ | ✅ | ✅ | ✅ |
| การแจ้งเตือน | ✅ | ✅ | ✅ scoped | ✅ scoped |
| รายงาน | ✅ | ✅ | ✅ scoped | ✅ scoped |
| นักเรียน (ดู) | ✅ | ✅ | ✅ scoped | ✅ scoped |
| นักเรียน (แก้ไข) | ✅ | ❌ | ❌ | ✅ scoped |
| แก้ไขเลขบัตร ปชช. | ✅ | ❌ | ❌ | ❌ |
| **กรอกแบบประเมินชั้นเรียน** | ❌ | ❌ | ❌ | ✅ |
| สังกัด/เขต/โรงเรียน | ✅ | ❌ | ❌ | ❌ |
| ผู้ใช้งาน | ✅ | ✅ | ❌ | ❌ |
| นำเข้าข้อมูล | ✅ | ❌ | ❌ | ✅ |
| **Audit Log** | ✅ | ✅ | ❌ | ❌ |
...
> **scoped** = เห็นเฉพาะข้อมูลในโรงเรียน/เขตของตัวเอง
